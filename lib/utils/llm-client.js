  ...
  if (assetType === 'harness' || assetType === 'agents') {
    try {
      ...
      if (fs.existsSync(kit.mcpTemplate)) {
        try {
          const mcpData = JSON.parse(fs.readFileSync(kit.mcpTemplate, 'utf-8'));
          const servers = mcpData.mcpServers || {};
          for (const key of Object.keys(servers)) {
            const s = servers[key];
            resources.mcp.push({
              name: key,
              command: s.command || 'custom-mcp',
              description: s.description || `MCP Server running command '${s.command || ''}'`
            });
          }
        } catch {
      ...
      if (resources.mcp.length > 0) {
        resourcesContext += '  <mcp_servers>\n';
        for (const m of resources.mcp) {
          resourcesContext += `    - Name: "${m.name}"\n      Command: "${m.command}"\n      Description: "${m.description}"\n`;
        }
        resourcesContext += '  </mcp_servers>\n';
      }
