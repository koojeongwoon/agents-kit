...
export function mergeStructuredDocument({
  ...
  if (format === 'jsonc' || format === 'jsonc-section') {
    return mergeJsonc({ current, desired, previousUnits });
  }
  if (format === 'yaml' || format === 'yaml-section') {
    return mergeYaml({ current, desired, previousUnits });
  }
