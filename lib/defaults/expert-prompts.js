...
3. 'Tool Usage & Boundaries': Explicitly state which skills, loops, and tools from the provided <available_local_resources> this agent is authorized to use. Include specific references to those files.
...
2. 'Local Resources & Router Index': Read the provided <available_local_resources> (skills and loops). For each active resource, create a clear, clickable Markdown file:// link referencing its absolute path. Instruct the agent when and how to dynamic-load and view these specific resource files based on the task type. Keep the main Agent.md document thin as a routing hub.
